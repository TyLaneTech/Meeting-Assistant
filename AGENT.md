...
| `core/reanalysis_guard.py` | Crash safety for the one operation that deletes a transcript before it has a replacement: writes `backups/{sid}/reanalysis-in-progress.json` before the clear, removes it on success, and `pending_session_ids()` is what the startup sweep rolls back. Separate from the trim/split snapshot on purpose |
| `core/browser.py` | Opens the UI as an app window (Chrome/Edge `--app=` or the installed PWA) instead of a browser tab |
| `core/app_window.py` | One app window. Every click that means "show me the app" (tray, toast, Start Menu) raises the window that is already open and sends it where the click was going over the `navigate` SSE; only when there is nothing to raise does it call `core/browser.py`. Unconfigured it opens a window as before, so `app_window.configure()` in app.py is what puts it in charge |
| `core/window_focus.py` | Finds that window and raises it: on Windows a visible Chromium-class window, owned by a browser process, whose caption ENDS with `Meeting Assistant` (an ordinary browser window and an Electron editor both append their own name, so they are skipped); on macOS the same match through AppleScript. Best effort, never raises |
| `core/calendar_feed.py` | Published-calendar (ICS) download, RRULE expansion, time zones, attendee parsing, URL masking |
| `core/calendar_sync.py` | Matches recordings to calendar instances, stores the match and expected speaker count, feeds attendee candidates to the Speakers dialog's picker, runs the hourly refresh, and answers `title_for_start()` for the opt-in calendar naming of a new recording |
...
| File | Responsibility |
|---|---|
| `agent_api/rest.py` | Flask blueprint mounted at `/api/agent/v1`: meetings, transcripts (5 formats), frames, audio clips, hybrid search, folders (create, rename, move), bulk meeting moves, speakers (the work queue, the per-meeting evidence pack, frames while a speaker talks, labelling, per-line reattribution, profile detail / rename / confirmed merge / health, plan-confirm-apply relabel), settings, logs, live tail, opt-in recording control. Auth gate (`agent_api_enabled` / `agent_api_token` settings). |
...
| `agent_api/speakers.py` | Read-only speaker identification evidence: the per-key inventory with a status, identifying quotes and self-introductions, "thanks, Name" hints, frame moments, voice centroids, library matches with the margin verdict, in-meeting proximity, calendar attendees against assigned names, and the disclosures. Duck-typed over the voice library so tests pass a fake. |
| `agent_api/context.py` | `AgentContext`: callables injected by app.py (status payload, `_scope_filters`, `_describe_session`, model/AI snapshots, SSE push, and the speaker write paths: `label_speaker` = `_patch_session_speakers`, `apply_speaker_corrections`, `relabel_segment`, `rename_profile`, `merge_profiles`, `relabel_deps`) so the blueprint never imports app.py. |
...
| `library_changed` | `reason`, ... | The Agent API renamed or moved folders or meetings, or applied a bulk relabel; the UI runs `refreshSidebar()` and invalidates analytics and attention |
| `capture_alert` | `kind`, `message`, ... | The desktop capture has produced no signal (loopback silence watchdog) |
| `transcription_backlog` | `session_id`, `pending_sec`, `draining`, `dropped_chunks?` | Live transcription is behind the recording (`draining: false`) or is finishing the backlog after a stop (`draining: true`). `pending_sec: 0` takes the banner down |
| `reanalysis_rolled_back` | `session_id`, `segments`, `reason` | A reanalysis was interrupted and the previous transcript has been put back |
...
| `navigate` | `url` (a path and query) | The desktop is sending the open window somewhere, so a tray item or a toast with a destination raises that window instead of opening a second one to show it. The listener hands the URL to `navigateTo()` (`core/app_window.py`) |
...
**The MCP server dies with its client:** `serve()` exits on stdin EOF, which covers a clean client shutdown, and that alone is not enough on Windows. When an MCP client is force-killed, a sibling process that inherited the write end of the server's stdin pipe holds it open, the pipe never breaks, `readline()` blocks forever, and `mcp_server.py` survives with a dead parent until reboot; they accumulated across days of editor and terminal sessions before 2026-09-10. `_start_client_watchdog()` waits on the client's process handle in a daemon thread and calls `os._exit(0)` when it is signalled. Two details are load-bearing: a uv venv on Windows makes `.venv\Scripts\python.exe` a *trampoline* that runs the real interpreter as a child, so the client is the **grandparent** (detected exactly, via `sys.executable != sys._base_executable`, rather than by walking up anything Python-shaped: an MCP client that is itself Python must be watched, not walked through); and the watchdog must never be able to take down a healthy server, so every failure inside it is logged and swallowed. Covered by `tests/test_mcp_server_lifecycle.py`, which reproduces the inherited-pipe leak. Keep the watchdog stdlib-only like the rest of the file (`ctypes`, no `psutil`).
...
**Reanalysis is reversible:** `_run_reanalysis` clears the transcript before it has a replacement and the worker is a daemon thread, so anything that interrupted the rebuild left the meeting empty, permanently, with no snapshot and no restore path. `core/reanalysis_guard.py` writes `backups/{sid}/reanalysis-in-progress.json` before `reset_session_transcript` and deletes it the moment the batch pipeline returns, so the file's presence means exactly one thing: a pass started and did not finish. `_rollback_reanalysis()` restores it on the exception path, on `_force_quit` and on both restart paths; `_rollback_interrupted_reanalyses()` sweeps leftovers at startup, ahead of the model threads. The bias is towards restoring (a spurious rollback costs a rebuild the user can re-run; a missed one is unrecoverable), and the one snapshot never restored is an empty one. `storage.restore_session_transcript()` is the exact inverse of `reset_session_transcript`, **including the FTS rows and `global_id`**, and deliberately narrower than `restore_session_snapshot`, because restoring the summary or chat would undo edits made while the pass ran. Keep it off `session-original.json`: that file backs the user-visible trim/split "restore original" and must never be consumed, overwritten or deleted here. Covered by `tests/test_reanalysis_rollback.py`.
...
**One app window:** every click that means "show me the app" goes through `core/app_window.py`, which raises the window that is already open and only opens one when there is nothing to raise. Chromium is the reason it takes a window manager to do this: `--app=<url>` makes a new window every time, and only an installed PWA (`--app-id`) focuses the window it has, so a user without the PWA collected a window per toast. `core/window_focus.py` does the finding, and its match is deliberately narrow: a visible Chromium-class window, owned by a browser process, whose caption **ends** with `Meeting Assistant`. All three are load-bearing. The caption alone matches a browser tab, the class alone matches every Electron app on the machine (an editor open in a folder of this name is the false positive that motivated the process check), and an ordinary browser window appends the browser's own name to the title, which is what the suffix rule uses to skip it. A click that carries a destination (Settings, a meeting) sends the raised window there over the `navigate` SSE and needs a client connected to do it, so `client_count() > 0` gates that case; a click with no destination (`navigate=False`: the tray icon, an autostart toast) raises the window where it stands, because sending a window watching a live recording back to Home is worse than the second window ever was. Unconfigured, `app_window.show()` opens a window exactly as before, which keeps a unit test that imports the tray from grabbing the developer's desktop.
...
**The Speakers dialog is one surface, Cleanup:** Resolve was folded into it (2026-09-04) and Manage followed (2026-09-17), so there is no tab strip, no `switchSpeakerManagerTab` and no second commit model. Manage's only job Cleanup could not already do was picking a speaker's colour; that is now the colour square in each group header, which opens a palette popover through the one `_cleanupPicker` slot (so Escape and outside-click are already handled). The colour is **staged** like every other Cleanup edit: `_cleanupSetClusterColor` sets `cluster.color`, marks the state dirty and repaints, and `Apply` is what writes it. `apply_cluster_corrections` used to read the colour back out of the profile row, which silently dropped a staged recolour, so pass 1b calls `rename_global_speaker(gid, color=)` first: the colour belongs to the Voice Library profile and follows the person into every meeting, matching what the Voice Library's own colour grid already did. A group with neither a profile nor a typed name has nothing to hang a colour on, so its swatch is disabled rather than offering a pick that Apply would drop.

**The selection bar is anchored to the scroll area, not the pane:** it was absolutely positioned against `.speaker-cleanup-body`, so selecting a speaker put it on top of the Apply footer and people reported Apply as missing (2026-09-17). `.cleanup-scroll-wrap` is its positioning context now and ends above the help text, so the bar floats over the cards and never over the commit row. Keep any new floating affordance inside that wrapper.
